...
{
    float coef = ENEMY[enemy->id].speed * difficulty
...
{
    if (enemy->cooldown <= 0) {
        save->info->item_info[INFO_HEALTH] -= ENEMY[enemy->id].attack;
        enemy->cooldown = ENEMY[enemy->id].cooldown *
...
void enemies_movement(game_t *game, linked_list_t *enemies, save_t *save)
{
    ...
    for (node_t *head = enemies->head; head != NULL; head = head->next) {
        ...
        if (tmp->dist > RENDER_DISTANCE * save->info->difficulty
            || tmp->id < NB_ITEM)
            continue;
        if (tmp->dist < ENEMY[tmp->id].attack_range * save->info->difficulty)
