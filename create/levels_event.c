...
static int get_map_name(maps_t *maps, system_t *sys)
{
    ...
    if (maps->str < LEVELS_SAVE1 || maps->str > LEVELS_SAVE3)
        return SUCCESS;
    if (maps->str == LEVELS_SAVE1)
        sys->save->name = strdup(((file_t *)head->data)->name);
    if (maps->str == LEVELS_SAVE2)
        sys->save->name = strdup(((file_t *)head->next->data)->name);
    if (maps->str == LEVELS_SAVE3)
    ...
    system_t *sys, state_info_t *state)
{
    ...
    if (is_input(event, sfKeyEscape, sfTrue, 7)) {
        state->old_scene = state->scene;
        state->scene = maps->draw[LEVELS_BACK].scene;
        free_linked_list(maps->info->list, &free_node_file);
        maps->info->list = NULL;
        maps->info->update = sfFalse;
        maps->draw[maps->str].color = sfWhite;
...
static void check_save_minus(maps_t *maps)
{
    ...
    if (tmp == LEVELS_SAVE2 && head->next->next != NULL)
        tmp++;
    if (maps->str > tmp && maps->str < LEVELS_BACK)
        maps->str = tmp;
}
...
static void check_save_plus(maps_t *maps)
{
    ...
    if (maps->str == LEVELS_SAVE1 && head == NULL)
        maps->str = LEVELS_BACK;
    if (maps->str == LEVELS_SAVE2 && head->next == NULL)
        maps->str = LEVELS_BACK;
    if (maps->str == LEVELS_SAVE3 && head->next->next == NULL)
        maps->str = LEVELS_BACK;
}
...
static void switch_str(sfEvent event, maps_t *maps)
{
    ...
    if (maps->str == NB_LEVELS)
        maps->str = LEVELS_SUB;
    if (maps->str == LEVELS_TITLE)
        maps->str = LEVELS_BACK;
...
{
    if (maps->str >= LEVELS_SUB && maps->str <= LEVELS_SAVE3) {
        move_map(event, maps);
        maps->draw[maps->str].color = sfRed;
    }
}
